...
#   table column widths
    ...
    print("closed and merged PRs in master")
    merged_features = []
    merged_fixes = []
    ...
    for pr in pulls:
        if pr.is_merged():
            ...
            if pr_merge_date > prev_release_commit_date:
                labels = pr.labels
                label = [l.name for l in labels if l.name=='type:new-feature' or l.name=='type:enhancement']
                #label = labels
                if label:
                    pr_num = str(pr.number)
                    merged_features = merged_features.append(pr_num)
                label = []
                label = [l.name for l in labels if l.name=='type:bug' or l.name=='BUG']
                if label:
                    pr_num = str(pr.number)
                    merged_fixes = merged_fixes.append(pr_num)
